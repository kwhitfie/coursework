            ...
            <tr> <td> <b>Name </th> <td> {{$event->name}}</td></tr>
              ...
              <table><tr>
                <td><a href="/list" class="btn btn-primary" role="button">Back to the list</a></td>
                  <td><a href="{{action('App\Http\Controllers\EventController@edit', $event['id'])}}" class="btn btn- warning">Edit</a></td>
                    <td><form action="{{action('App\Http\Controllers\EventController@destroy', $event['id'])}}"
                      method="post"> @csrf
